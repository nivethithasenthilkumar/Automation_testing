...
import com.nivethithasenthilkumar.automation.utils.*;
...
public class TestSimpleCode {
    ...
    public void testRemoveAccent() {
        WebUI.logConsole(LanguageUtils.removeAccent("Nivethitha Senthilkumar"));
    }

    @Test
    public void testMakeSlug() {
        WebUI.logConsole(SystemHelpers.makeSlug("Nivethitha Automation Testing"));
    }
...
//        PropertiesHelpers.setValue("base_url", "https://anhtetser.com");
    ...
    public void testReadAndWriteTxtFile() {
        PropertiesHelpers.loadAllFiles();
        //Read all data
        WebUI.logConsole(FileHelpers.readTxtFile(PropertiesHelpers.getValue("TXT_FILE_PATH")));
        //Read data by line number
        WebUI.logConsole(FileHelpers.readLineTxtFile(PropertiesHelpers.getValue("TXT_FILE_PATH"), 0));
        WebUI.logConsole("-----------------------------");
        FileHelpers.writeTxtFile(PropertiesHelpers.getValue("TXT_FILE_PATH"), "\nTest123");
        WebUI.logConsole(FileHelpers.readTxtFile(PropertiesHelpers.getValue("TXT_FILE_PATH")));
